...
test("非表示にした名前は候補から外れる", () => {
  const state = makeState([
    makeTask({ title: "筋トレ", createdAt: 10 }),
    makeTask({ title: "散歩", createdAt: 20 }),
  ]);

  assert.deepEqual(selectTaskNameSuggestions(state, { excluded: ["筋トレ"] }), ["散歩"]);
});

test("非表示リストの追加と解除が保存を往復する", (t) => {
  ...
  assert.deepEqual(addSuggestionExclusion("筋トレ"), ["筋トレ"]);
  assert.deepEqual(addSuggestionExclusion(" 筋トレ "), ["筋トレ"]);
  assert.deepEqual(readSuggestionExclusions(), ["筋トレ"]);
  assert.deepEqual(clearSuggestionExclusion("筋トレ"), []);
  assert.deepEqual(readSuggestionExclusions(), []);
});
